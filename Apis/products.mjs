...
router.post("/product", async (req, res) => {
  ...
  if (
    ...
    res
      .status(200)
      .send({ message: "product added successfully", data: results });
    return;
  }
...
router.get("/products", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send({ data: results, message: "getting all products successfully" });
  } catch (err) {
    res
      .status(501)
      .send({ message: "error in sending all products from server" });
  }
});
...
router.put("/product/:id", async (req, res) => {
  let id = req.params.id;
  let { productPrice, productDescription, productCompany, productName } =
    req.body;
